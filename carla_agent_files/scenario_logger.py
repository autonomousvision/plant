...
class ScenarioLogger():
  ...
  def _initialize_bg_agents(self):
    ...
    for tlight in tlights:
      if tlight.state != carla.libcarla.TrafficLightState.Green:
        trigger_box_global_pos = tlight.get_transform().transform(
            tlight.trigger_volume.location)
  ...
  def log_step(self, route, ego_control=None):
    ...
    if not self.ego_yaw:
      self.ego_yaw = self.ego_vehicle.get_transform()
      self.ego_yaw = np.array([[[np.radians(self.ego_yaw.rotation.yaw)]]])
    ...
    if len(self.bg_vehicles) > 0:
      state = {
          "pos":
              np.concatenate([self.ego_pos, self.bg_pos], axis=1).tolist(),
          "yaw":
              np.concatenate([self.ego_yaw, self.bg_yaw], axis=1).tolist(),
          "vel":
              np.concatenate([self.ego_vel, self.bg_vel], axis=1).tolist(),
          "extent":
              np.concatenate([self.ego_extent, self.bg_extent],
                             axis=1).tolist(),
          "id":
              np.concatenate([self.ego_id, self.bg_id], axis=1).tolist(),
      }
    else:
      state = {
  ...
  def fetch_bg_state(self):
    ...
    for tlight in self.tlights:
      ...
      center_bounding_box = tlight.get_transform().transform(
          tlight.trigger_volume.location)
      center_bounding_box = carla.Location(center_bounding_box.x,
                                           center_bounding_box.y,
                                           center_bounding_box.z)
      length_bounding_box = carla.Vector3D(tlight.trigger_volume.extent.x,
      ...
      # shape is batch_size x num_agents x state_dims
      tl_positions.append(
          np.array([[[center_bounding_box.x, center_bounding_box.y]]]))
  ...
  def route_as_boxes(self, route):
    ...
    for i, midpoint in enumerate(midpoints):
      ...
      start_bounding_box = carla.Location(shortened_route[i][0],
                                          shortened_route[i][1], 0.0)
      if 0 < i < 10 and start_bounding_box.distance(
          self.ego_location) > self.roi:
        continue

      length_bounding_box = carla.Vector3D(
          norms[i] / 2., self.ego_vehicle.bounding_box.extent.y,
  ...
  def dump_to_json(self):
    ...
    """
    if not self.route_only:
      self.records_file_path = os.path.join(self.save_path, "records.json")
