...
from training.Perception.utils import *
from training.Perception.transfuser import TransfuserBackbone, SegDecoder, DepthDecoder
from training.Perception.point_pillar import PointPillarNet
...
class LidarCenterNet(nn.Module):
    ...
    def __init__(self, config, device, backbone, image_architecture='resnet34', lidar_architecture='resnet18', use_velocity=True):
        ...
        self.device = device
        self.config = config
        self.pred_len = config.num_route_points
        ...
        if config.light_weight > 0:
            self.BCE_loss = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(config.light_weight).to(self.device))
        else:
        ...
        if self.config.learn_origin:
            join_output_features = 66
        else:
            join_output_features = 64
        ...
        # self.decoder = nn.GRUCell(input_size=7 if self.gru_concat_target_point else 5, # 2 represents x,y coordinate
        #                           hidden_size=self.config.gru_hidden_size).to(self.device)
        
        self.decoder = nn.GRUCell(input_size=2 + target_point_size, # 2 represents x,y coordinate
                                  hidden_size=self.config.gru_hidden_size).to(self.device)

        self.tanh = nn.Tanh()
        
        self.traffic_light_detector = nn.Sequential(
                    nn.Linear(join_output_features, 32),
                    nn.ReLU(inplace=True),
                    nn.Linear(32, 1),
                ).to(self.device)

        # self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # self.output = nn.Linear(self.config.gru_hidden_size, 5).to(self.device)
    ...
    def forward_gru(self, z, target_point):
        ...
        # x = torch.zeros(size=(z.shape[0], 5), dtype=z.dtype).to(z.device)
         # initial input variable to GRU
        if self.config.learn_origin:
            x = z[:, 64:66]  # Origin of the waypoints
            z = z[:, :64]
        else:
        ...
        for _ in range(self.pred_len):
            ...
            # for position predict the offset for sintheta, cosine theta and length of bounding box predict absolut values
            # x = torch.cat((dx[:,:4] + x[:,:4], dx[:,4:]), dim=1)
            x = dx[:,:2] + x

            # x[:,2:4] = self.tanh(x[:,2:4]) # sin and cos should be between -1 and 1
            
            # output_wp.append(x)
            output_wp.append(x[:,:2])

        # predictions = torch.stack(output_wp, dim=1)
        # pred_wp = predictions[:,:,:2]
        # pred_route = predictions[:,:,2:] # TODO: check coordinate system
        ...
        return pred_wp, pred_route, pred_brake, steer, throttle, brake
    ...
    def forward_ego(self, rgb, lidar_bev, target_point, target_point_image, ego_vel, bev_points=None, cam_points=None, save_path=None, expert_waypoints=None,
                    stuck_detector=0, forced_move=False, num_points=None, rgb_back=None, debug=False, bb_confidence_threshold=None,
                    tl_threshold=None):
        ...
        fused_features_64 = self.join(fused_features)

        pred_traffic_light = F.sigmoid(self.traffic_light_detector(fused_features_64))
        pred_traffic_light = pred_traffic_light.squeeze().item() > tl_threshold
        pred_wp, pred_route, _, _, _, _ = self.forward_gru(fused_features_64, target_point)
        ...
        if bb_confidence_threshold is None:
            bb_confidence_threshold = self.config.bb_confidence_threshold
        bboxes = bboxes[bboxes[:, -1] > bb_confidence_threshold]
        rotated_bboxes = []
        ...
        self.i += 1
        if debug and self.i % 20 == 0 and not (save_path is None):
            
            self.visualize_model_io(save_path, self.i, self.config, rgb, lidar_bev, target_point,
                            pred_wp, bboxes, self.device,
                            gt_bboxes=None, expert_waypoints=expert_waypoints, stuck_detector=stuck_detector, forced_move=forced_move)


        return pred_wp, rotated_bboxes, bboxes, pred_route, pred_traffic_light

    def forward(self, rgb, lidar_bev, ego_waypoint, target_point, target_point_image, properties_route, ego_vel, traffic_light, label, num_points=None, save_path=None, cam_points=None):
        loss = {}
        ...
        # get 64 dim feature vector
        fused_features_64 = self.join(fused_features)

        pred_traffic_light = self.traffic_light_detector(fused_features_64)
        tl_loss = self.BCE_loss(pred_traffic_light, traffic_light)
        
        pred_wp, pred_route, _, _, _, _ = self.forward_gru(fused_features_64, target_point)

        # pred topdown view
        loss_wp = torch.mean(torch.abs(pred_wp - ego_waypoint))
        loss_route = torch.mean(torch.abs(pred_route - properties_route))
        loss.update({
            "loss_wp": loss_wp,
    ...
    def visualize_model_io(self, save_path, step, config, rgb, lidar_bev, target_point,
        ...
        cv2.imwrite(str(save_path + ("/rgb%d.png" % (step // 2))), rgb_image)
        cv2.imwrite(str(save_path + ("/im%d.png" % (step // 2))), images)